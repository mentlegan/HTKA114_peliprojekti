...
func nayta_journal_info():
	...
	if not valossa:
		journal_info_label.text = "It's too dark to read the journal"
	if pimeyskuolema.is_playing() or pimeyskuolema.modulate.a > 0:
		journal_info_label.text = "That would be too dangerous!"
...
func siirrytty_veteen():
	...
	if not audio_uinti.playing: # Uimisen ääni
		audio_uinti.play()
		# Viedään "Kaiku"-audioväylän ja musiikin äänet "Veden_alla" audioväylän läpi Masteriin
	AudioServer.set_bus_send(AudioServer.get_bus_index("Kaiku"), "Veden_alla")
...
func soita_huilua():
	...
	# Laitetaan illuusiot pois päältä hetkeksi, kun soitetaan huilua vihreällä taajuudella
	if aanen_taajuus == 2:
...
func tallenna():
	return {
		...
		# Jos pelkän muuttujan asettaminen ei riitä noden tilan palauttamiseen,
		# voi kirjoittaa erillisen funktion lataa().
		# Globaali.gd kutsuu lataa-funktiota, jos sellainen on olemassa.
...
## Tarkistaa pelaajan poistuvan vedestä
...
## Tarkistaa pelaajan siirtyvän veteen
...
## Tarkistaa pelaajan hapen oton
func _on_hapen_tarkistus_area_entered(area):
	if area is Happikukka and vedessa: # Jos ollaan vedessä, ja pelaajan hapen tarkistus huomaa happikukan..
		tayta_happi() # .. happi täyttyy


## Tarkistaa, kun pelaaja ei ota happea
func _on_hapen_tarkistus_area_exited(area):
	if area is Happikukka and vedessa:
		#siirrytty_veteen()
		happi_ajastin.start()
